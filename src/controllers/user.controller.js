...
const registerUser = asyncHandler(async (req, res) => {
  const { name, shopName, shopAddress,email, password,mobileNumber,role } = req.body;
 
  //  All fields non-empty validation
  if (
    [name, shopName, shopAddress,email, password,mobileNumber].some((field) => field?.trim === "")
  ) {
    throw new ApiError(400, "Please fill all the fields");
  }

  const isUserExists = await User.findOne({ $or: [{ mobileNumber }, { email }] });
  if (isUserExists) {
...
export { registerUser, getAllUsers,loginUser, logoutUser, refreshAccessToken ,changeCurrentPassword,getCurrentUser,updateAccountDetails,updateUserCoverImage,updateUserAvatar,getUserChannelProfile,getWatchHistory};
